...
class ContentProviderMailerTest < ActionMailer::TestCase
  test "notify_content_provider" do
    event = events(:one)
    content_provider = content_providers(:approval_notification_email_contact_provider)

    mail = ContentProviderMailer.notify_content_provider(event, content_provider)
  ...
  test "does not send if approval_notification_email is blank" do
    event = events(:one)
